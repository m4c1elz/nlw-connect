...
class SubscribersCreator:
    ...
    def __check_sub(self, subcriber_email: str, evento_id: int) -> None:
        response = self.__subs_repo.select_subscriber(subcriber_email, evento_id)
        if response:
            raise Exception("Subscriber already exists!")

    def __insert_sub(self, subscriber_dict: dict) -> None:
        self.__subs_repo.insert(subscriber_dict)
